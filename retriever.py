...
def retrieve(query, n_results=N_RESULTS):
    ...
    Return a list of dicts, each with:
      - "text"     : the chunk text
      - "source"     : the source name (pull this from metadatas)
    ...
    return [
        {"text": chunks[i], "source": metadatas[i]["source"], "distance": distances[i]}
        for i in range(len(chunks))
    ]

def test_retrieval(queries):
    """Run sample queries and print chunks with distance scores."""
    print(f"\nCollection has {_collection.count()} chunks total.\n")
...
if __name__ == "__main__":
    ...
    sample_queries = [
        "What is the university-approved apartment near campus?",
        "What are the most popular apartments for students?",
        "Do all students live off campus?",
    ]
    test_retrieval(sample_queries)
